...
sudo apt install -y wget gpg python3-pip golang-go curl unzip ffmpeg build-essential # NOSONAR

# Check for Cargo (Rust)
if ! command -v cargo &> /dev/null; then
    echo -e "${c}Cargo not found. Installing Rust...${r}"
    curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh -s -- -y # NOSONAR
...
if ! command -v cargo-binstall &> /dev/null; then
    ...
    curl -L --proto '=https' --tlsv1.2 -sSf https://raw.githubusercontent.com/cargo-bins/cargo-binstall/main/install-from-binstall-release.sh | bash # NOSONAR
fi
...
if ls "$FONT_DIR"/FiraCode*.ttf 1> /dev/null 2>&1; then
    ...
    wget -qO /tmp/FiraCode.zip https://github.com/ryanoasis/nerd-fonts/releases/latest/download/FiraCode.zip # NOSONAR
