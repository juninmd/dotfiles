...
case "$PROFILE" in
  minimal)
    DEFAULT_MODULES=(bat bottom btop choose circumflex cli-tools elixir eza fd-find fzf gleam gobang lsd macchina nix ripgrep sd starship vscode xplr zig zoxide zsh)
    ;;
  dev)
    DEFAULT_MODULES=(atuin bat biome bottom broot btop bun choose circumflex cli-tools dagger deno devbox docker elixir eza fd-find fzf gitui gleam glow gobang kdash lazydocker lazygit lazynpm lsd macchina mise mysql nap neovim nix procs pueue ripgrep ruff sd slumber starship uv vscode xplr yazi zellij zig zoxide zsh)
    ;;
  full)
    DEFAULT_MODULES=(act actionlint age aichat aider amber android argc argocd ast-grep atac atlas atuin awscli bacon bandwhich bat bat-extras binsider biome bluetuith bore-cli bottom brave broot bruno btop bun bw carapace cbonsai chafa charm chatbox chatgpt-cli cheat checkov choose circumflex claude-code cli-tools cline cloudflared cocogitto code2prompt cointop consul cpufetch crane croc csvlens ctop curlie cursor czg d2 dagger dapr dasel daytona dbeaver dbmate delta deno devbox devenv devpod difftastic direnv discord diskonaut distrobox dive docker doggo dolt doppler dotenv-linter dotenvx dprint dsq dua dua-cli duckdb duf dufs dura dust dysk earthly eget elixir erdtree evans eza fabric fastfetch fd-find fend firefox flox flyctl fnm fq freeze fx fzf gcloud gdu genact gh gh-dash ghostty ghq git-absorb git-cliff git-filter-repo git-sim git-town gitingest gitleaks gitui glab glances gleam glow gobang gojq gping grex gron grpcurl grype gtt gum harlequin hck helix helm heroku hexyl howdoi htmlq httpie httpstat httpx hurl hwatch hyperfine igrep infisical infracost inlyne inshellisense jan jaq jc jira-cli jj jless jnv jo joshuto jq jql jqp jujutsu just k3d k3s k6 k8sgpt k9s kalker kdash kind klog kmon ko kondo krew kubecolor kubectl kubectx kubens kustomize lazydocker lazygit lazynpm lazysql lefthook lf llm lmstudio lnav lsd lychee macchina mani mcfly mdcat melt miller miniserve mise mkcert moar mods monolith moon mprocs mysql nap navi ncspot neovim netlify newsboat ngrok nix nomad nuclei numbat nushell obsidian oha ollama onefetch open-interpreter opentofu ouch oxker oxlint packer pastel peco pipes-rs pipes-sh pkgx plandex pnpm podman pokeget pomsky popeye porsmo posting presenterm procs pueue pulumi px qsv repomix rip ripgrep ripgrep_all rnr rs-cmatrix ruff ruplacer rustscan rye sad scc sd serie serpl sesh shell-gpt shellcheck shfmt silicon skate skim slack slides slumber sniffnet so sops spacer spt sqlc starship steampipe stern stripe supabase superfile syft systemctl-tui systeroid sysz t-rec tailspin taplo task taskwarrior-tui tealdeer television tenki tenv termdbms termscp termshark termtyper terragrunt tflint tfsec tgpt thefuck tickrs tilt tin-summer tldr tlrc tokei topgrade trash-cli tre trippy trivy trufflehog trzsz tt ttyd ttyper turso typos typst ugrep usql uv vault vcluster vegeta vercel vhs viddy visidata viu vscode walk warp watchexec websocat wezterm wiki-tui windsurf wtfutil wthrr wuzz xc xcp xh xplr xsv yamlfmt yazi yq yt-dlp zed zellij zen-browser zenith zig zizmor zoxide zrok zsh)
    ;;
  ai-dev)
    DEFAULT_MODULES=(aichat aider atac atuin bat biome bottom broot bruno btop bun cbonsai chatbox choose circumflex claude-code cli-tools cline code2prompt curlie cursor dagger daytona dbeaver deno devbox devpod docker doggo dsq duckdb elixir eza fabric fastfetch fd-find fzf gitingest gitui gleam glow gobang harlequin inshellisense jan jless jo k6 k8sgpt k9s kdash lazydocker lazygit lazynpm lazysql llm lmstudio lsd macchina mise mods mprocs nap neovim nix oha ollama open-interpreter pipes-sh plandex podman posting procs pueue repomix ripgrep ruff sd shell-gpt slumber starship superfile t-rec television tgpt tokei uv warp wezterm windsurf xplr yazi zed zellij zen-browser zig zoxide zsh)
    ;;
  *)
